...
export const MatchoryResultList = ({ data, onBookmarkSupplier }) => {
  ...
  const getmapCooridnates = (coords) => {
          ...
          {data &&
            data.suppliers.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`supplier-list-item-container flex gap-[12px] p-[12px] h-[88px] cursor-pointer hover:bg-[#F8F8F8] ${
                    isLastItem ? '' : 'border-b'
                  } border-gray-200 ${isFirstItem ? '' : 'border-t'}`}
                  onClick={() => getmapCooridnates(item.location)}
                >
                    ...
                    <div className="sup-details-div flex flex-row">
                      <div className="mid-container-details flex">
                        ...
                        <div className="loc font-inter font-[400] text-[16px]">
                          <span>{item.location.locname}</span>
                        </div>
                      </div>
                  ...
                  <div className="icon-holder ml-auto ">
                    <button onClick={() => onBookmarkSupplier(item.location)}>
                      {item.isSelected ? (
                        <img
